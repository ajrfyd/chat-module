...
window.onload = (e) => {
  ...
  socketPlugin.createNickName = (name) => {
    ...
    socket.on("nickname-exist", () => invalidHandler(qs(".backdrop .nickname-input"), "이미 존재하는 닉네임 입니다."));
    socket.on("nickname-created", (data) => {
      store.dispatch(setUserData(data));
      localStorage.setItem("nickName", data.nick_name);
      removeNickNameInput();
      socket.off("nickname-exist", () => invalidHandler(qs(".backdrop .nickname-input"), "이미 존재하는 닉네임 입니다."));
    });
  };
